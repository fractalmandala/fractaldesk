...
- [Notes Manager Implementation](docs/worklog/notes-manager-impl.md) — new `notes` surface from approved product/design/tech specs (specs at `docs/specs/notes-manager/`): multi-root tree, note list, raw (CodeMirror 6) plus rich (Milkdown Crepe) editor, info sidebar, spec-authoritative `file://` links; 16 confined `notes_*` Rust commands with atomic writes plus `notify` watcher; `pnpm check` 0 errors, vitest 21/21, `cargo test` 34/34, `pnpm build` succeeds. Search specced but deferred; manual acceptance in the running app still open.
