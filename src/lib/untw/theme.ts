...
export function emptyTheme(): ThemeInfo {
  return { colors: new Set(), radii: new Set(), animates: new Set(), fonts: new Set(), texts: new Set(), detail: {}, light: {}, dark: {}, keyframes: new Set() }
}
...
export function parseModeValues(css: string): Pick<ThemeInfo, 'light' | 'dark'> {
  ...
  for (const [sel, target] of [[':root', light], ['.dark', dark]] as const) {
...
export function concreteValues(varName: string, t: ThemeInfo): { light: string | null; dark: string | null } {
