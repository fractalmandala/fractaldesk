...
export function decode(code: string, themeCss = ''): DecodeResult {
  ...
  return {
    blocks, tokens, summary, unknowns, unresolvable, markers,
...
export function summaryText(r: DecodeResult): string {
  ...
  const lines = order.filter((c) => r.summary[c]?.length).map((c) => `${c}: ${r.summary[c].join(' ')}`)
...
export function tokenLines(r: DecodeResult): string {
  return r.tokens
    ...
    .map((t) => `${t.token} -> ${t.css}${t.conditions.length ? `  [${t.conditions.join('; ')}]` : ''}`)
    .join('\n')
}
