...
const LINEAR: Record<string, string> = {
  ...
  'min-h': 'min-height', 'max-h': 'max-height', basis: 'flex-basis'
}
...
const COLOR_PROPS: Record<string, string> = {
  ...
  ring: '--tw-ring-color', divide: 'border-color',
...
const COLOR_PREFIX = 'bg|text|border-(?:[trblxyse])|border|ring|divide|decoration|outline|accent|caret|fill|stroke'

/** Concrete color literal (not a var() indirection)? usable for srgb fallbacks. */
function isColorLiteral(value: string): boolean {
  const v = value.trim().toLowerCase()
  return /^(oklch|oklab|rgb|hsl|lab|lch|color-mix|light-dark)\(/.test(v) || /^#[0-9a-f]{3,8}$/.test(v)
}
...
function synthesizeOpacity(core: string, theme: ThemeInfo): { css: string; fallback: string | null } | null {
  ...
  if (isTheme) {
    ...
    fallback = detail && isColorLiteral(detail.value)
...
function synthesizeArbitrary(core: string): string | null {
  ...
  const pre = arb[1]
  const raw = arb[2]
  const val = raw.startsWith('[') ? raw.slice(1, -1).replace(/_/g, ' ') : `var(${raw.slice(1, -1)})`
  const prop =
    LINEAR[pre] ??
    { 'min-w': 'min-width', 'max-w': 'max-width', 'min-h': 'min-height', 'max-h': 'max-height', origin: 'transform-origin', size: null as unknown as string }[pre]
  if (!prop) {
...
function unknownHint(core: string, variants: string[], theme: ThemeInfo): string {
  ...
  for (const prefix of ['bg-', 'text-', 'border-', 'ring-', 'from-', 'via-', 'to-', 'accent-', 'decoration-']) {
    if (core.startsWith(prefix)) {
      const rest = core.slice(prefix.length).split('/')[0]
      if (!KNOWN_COLORS.has(rest) && !theme.colors.has(rest))
        return theme.colors.size > 0 ? 'not in pasted theme — typo or missing @theme entry?' : 'custom theme color? paste @theme CSS and convert again'
    }
  }
  if (/^animate-/.test(core)) return 'custom animation? paste @theme CSS with its @keyframes and convert again'
  if (/^(placeholder|file|marker|selection)-/.test(core)) return 'v4 variant-position utility — write as placeholder:… / file:…'
...
export function resolveToken(token: string, theme: ThemeInfo): Resolved {
  const base = {
    token, variants: [] as string[], conditions: [] as string[], core: token,
    css: null as string | null, fallback: null as string | null, px: null as number | null,
    category: 'Other', fractal: null as string | null,
    unknown: false, marker: false, synthesized: false, hint: null as string | null
  }
  if (isDynamic(token)) {
    return { ...base, unknown: true, hint: 'dynamic value — cannot resolve statically; add to safelist' }
  }
  ...
  const themed = theme.colors.size + theme.radii.size + theme.animates.size + theme.fonts.size + theme.texts.size > 0
