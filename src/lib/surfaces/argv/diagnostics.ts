...
import type { Spec, Opt } from './spec'
import { argToken, byId, chain, isKebab, kids, optKey, pathOf, scopeOpts } from './spec.js'

interface Diag { s: string; m: string; c: string }

const RESERVED_LONG = new Set(['help', 'version'])
const RESERVED_SHORT = new Set(['h', 'V'])

export function diagnose(spec: Spec): Diag[] {
	const out: Diag[] = []
	const push = (s: string, m: string, c: string) => out.push({ s, m, c })
	...
	for (const c of spec.commands) {
		...
		const seenLong = new Map<string, Opt>()
		const seenShort = new Map<string, Opt>()
