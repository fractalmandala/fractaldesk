...
interface GeneratedFile { name: string; body: string }

const handlerName = (spec: Spec, c: Command): string => camel(chain(spec, c.id).map((n) => n.name).join('-'))
const modPath = (spec: Spec, c: Command): string => 'commands/' + chain(spec, c.id).map((n) => kebab(n.name)).join('-')
const optsType = (spec: Spec, c: Command): string => pascal(chain(spec, c.id).map((n) => n.name).join('-')) + 'Options'

function tsType(o: Opt): string {
...
export function generateEntry(spec: Spec): string {
	...
	const emit = (parentId: string, varName: string) => {
...
export function generateHandler(spec: Spec, c: Command): string {
	const ts = spec.pkg.language === 'ts'
...
function generateManifest(spec: Spec): string {
	const ts = spec.pkg.language === 'ts'
...
function generateReadme(spec: Spec): string {
...
export function generatePackage(spec: Spec): GeneratedFile[] {
	const ts = spec.pkg.language === 'ts'
	const ext = ts ? '.ts' : '.js'
	const out: GeneratedFile[] = [{ name: ts ? 'src/cli.ts' : 'src/cli.js', body: generateEntry(spec) }]
