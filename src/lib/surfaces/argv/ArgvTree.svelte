<script lang="ts">
	import type { Spec, Command } from './spec'
	import { kids, uid } from './spec'

	let { spec, sel = $bindable() }: { spec: Spec; sel: string } = $props()

	function rows() {
		const out: Array<{ id: string; name: string; depth: number; subs: number; glyph: string; count?: string }> = [
			{ id: 'root', name: spec.pkg.bin || 'cli', depth: 0, subs: kids(spec, 'root').length, glyph: '◆' }
		]
		const walk = (pid: string, depth: number) => {
			for (const c of kids(spec, pid)) {
				const n = kids(spec, c.id).length
				out.push({
					id: c.id, name: c.name || '?', depth, subs: n, glyph: n ? '▸' : '·',
	...
	function addCommand(parent: string | null) {
		const c: Command = { id: uid('c'), parent, name: 'command', alias: '', desc: '', args: [], opts: [] }
