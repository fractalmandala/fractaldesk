...
const matches = (c: Command, tok: string): boolean => c.name === tok || (!!c.alias && c.alias === tok)

interface CoerceResult { value?: unknown; error?: string }

function coerce(o: Opt, raw: string): CoerceResult {
	if (o.type === 'number') {
		const n = Number(raw)
...
export function parseInvocation(spec: Spec, line: string): ParseResult {
	...
	const byLong = new Map<string, Opt>()
	const byShort = new Map<string, Opt>()
	...
	const given = new Map<string, { value: unknown; o: Opt }>()
	const positionals: string[] = []
	let help = false
	let version = false

	const take = (o: Opt, valueFromEq: string | undefined, idx: number, list: string[]): number => {
		if (o.type === 'boolean') {
	...
	const opts: ParsedOpt[] = inScope.map((o) => {
