<script lang="ts">
	...
	import { argToken, byId, chain, isLeaf, kids, optFlags, uid } from './spec'

	let { spec, sel }: { spec: Spec; sel: string } = $props()
	...
	const TYPES: [string, string][] = [['boolean', 'flag'], ['string', 'string'], ['number', 'number'], ['enum', 'choice']]
	const ARITY = ['required', 'optional', 'variadic']

	const newOpt = (): Opt => ({ id: uid('o'), short: '', long: 'new-flag', type: 'boolean', value: '', def: '', choices: '', desc: '' })
