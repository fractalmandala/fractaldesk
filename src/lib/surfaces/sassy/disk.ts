// The "On disk" action, driven from the header toolbar. One native panel
// selects files and/or folders together; each matching source is converted and
// written back next to itself with the swapped extension. The result is
// reported into the shared `sassy.report` (shown in the surface) and the
// header status line.

import { invoke } from '@tauri-apps/api/core'
import { say } from '../../store.svelte.js'
import { sassToCssFile, cssToSass } from './convert.js'
import { errMsg } from './util.js'
import { sassy } from './state.svelte.js'
import type { Src, WriteItem, WriteReport } from './types.js'
