...
export function spacingPx(core: string): number | null {
  const m = core.match(/^(?:-?[mp][trblxyse]?|gap(?:-x|-y)?|space-[xy]|inset(?:-x|-y)?|top|right|bottom|left|start|end|basis|w|h|min-w|max-w|min-h|max-h|size)-(\d+(?:\.\d+)?)$/)
  if (!m) return null
...
export function suggestFractal(core: string): string | null {
  ...
  if (r) {
    const map: Record<string, string> = { xs: 'radius-2', sm: 'radius-4', md: 'radius-8', lg: 'radius-12', xl: 'radius-16', '2xl': 'radius-24', '3xl': 'radius-24' }
    return map[r[1]] ?? null
  }
