...
export function usageParts(spec, id) {
...
export const usageLine = (spec, id) => usageParts(spec, id).map((p) => p.v).join(' ')
...
export function helpLines(spec, id) {
  const node = byId(spec, id)
  const subs = kids(spec, id)
...
export const helpText = (spec, id) => helpLines(spec, id).map((l) => l.t).join('\n')
