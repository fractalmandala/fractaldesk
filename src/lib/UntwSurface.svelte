<script lang="ts">
	import { untw, runConvert, clearAll, copyText, SAMPLE, SAMPLE_ACCORDION } from './untw/state.svelte.js'
	import { summaryText, tokenLines } from './untw/decode.js'
	import { say } from './store.svelte.js'

	const ORDER = ['Layout', 'Spacing', 'Sizing', 'Border', 'Typography', 'Surface', 'Effect', 'State', 'Other']
...
<div class="untw">
	<div class="cols">
		<div class="pane">
			...
			></textarea>
			<p class="note">Custom colors and animations resolve from the pasted theme. Everything runs offline.</p>
		</div>
			...
			{:else}
				...
				{#each r.tokens.filter((t) => !t.unknown && !t.marker) as t}
					<div class="row">
						...
						<div class="css">{t.css}{#if t.px !== null}<span class="px"> ≈ {t.px}px</span>{/if}</div>
						{#if t.fallback}<div class="css fb">fallback: {t.fallback}</div>{/if}
						{#each themeVars(t.css ?? '') as v}
							{@const th = r.theme}
							<div class="modes"><span>{v}</span><span>light {th.light[v] ?? '—'}</span><span>dark {th.dark[v] ?? '—'}</span></div>
						{/each}
						<div class="tline">
				...
				{#if r.markers.length}
					...
					{#each r.markers as m}
						<div class="row"><span class="tok">{m.anchor}</span><span class="hintline">group anchor — no CSS of its own{m.usedBy.length ? ` · read by ${m.usedBy.join(', ')}` : ''}</span></div>
					{/each}
				{/if}
				...
				{#if r.unknowns.length}
					...
					{#each r.unknowns as t}
						<div class="row"><span class="tok">{t.token}</span><span class="hintline">{t.hint}</span></div>
					{/each}
				{/if}
				...
				<div class="footbtns">
					<button class="mini" onclick={copySummary}>{untw.copied === 'summary' ? 'Copied' : 'Copy summary'}</button>
					<button class="mini" onclick={copyTokens}>{untw.copied === 'tokens' ? 'Copied' : 'Copy tokens'}</button>
					<button class="mini" onclick={copyJson}>{untw.copied === 'json' ? 'Copied' : 'Copy JSON'}</button>
				</div>
			{/if}
		</div>
	</div>
...
.pane
	...
	min-width: 0
	gap: var(--space4)
	padding: var(--space6) 0 var(--space6) var(--space6)
	&:last-child
